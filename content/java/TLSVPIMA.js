...
var TLSVPIMA = function(
		...
		var _s = this;
		var prototype = TLSVPIMA.prototype;
		...
		*/
		_s.setUpIMA = function(){
			...
			if(!_s.adContainer){
				_s.adContainer =  new TLSVPDisplayObject("div");
		...
		_s.onAdsManagerLoaded = function(e){
			...
			if(prt._d.autoPlay_bl || prt._d.aom_bl || prt.isThumbClick_bl && TLSVPPassword.isCorect) _s.play(e);
		}
		...
		_s.onAdError = function(e){
			...
			prt.isIMA = false;
			_s.stop();
			if(_s.videoScreen_do.isSafeToBeControlled_bl || prt._d.autoPlay_bl || prt.isThumbClick_bl && TLSVPPassword.isCorect){
		...
		_s.onContentPauseRequested = function(e){
			...
			if(_s.controller_do){
				...
				if(window['TLSVPCC']) TLSVPCC.disableButton();
		...
		_s.onContentResumeRequested = function(){
			...
			if(_s.controller_do){
				if(window['TLSVPCC']) TLSVPCC.enableButton();
		...
		_s.onContentProgress = function(e){
			...
			curTime = TLSVPUtils.formatTime(d.currentTime);
			totalTime = TLSVPUtils.formatTime(totalTimeInSeconds);
	...
	/* set prototype */
	TLSVPIMA.setPrototype = function(){
		TLSVPIMA.prototype = null;
		TLSVPIMA.prototype = new TLSVPDisplayObject("div");
	};

	TLSVPIMA.prototype = null;
	window.TLSVPIMA = TLSVPIMA;
}(window));