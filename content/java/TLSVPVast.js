...
var TLSVPVast = function(
		...
		var _s = this;
		var prototype = TLSVPVast.prototype;
		_s.id = -1;
		...
		_s.setSource = function(source){
			...
			_s.load(source);
			data.dispatchEvent(TLSVPData.VAST_LOADING);
		}
			...
			_s.vastXHR.onreadystatechange = function(e){
				if(_s.vastXHR.readyState == 4){
					if(_s.vastXHR.status == 200){
						...
						var respObj = TLSVPUtils.xmlToJson(_s.vastXHR.responseXML);
						...
						var respObj = TLSVPUtils.xmlToJson(_s.vastXHR.responseXML).VAST;
						
						if(!respObj["Ad"]){
							data.dispatchEvent(TLSVPData.LOAD_ERROR, {text:"No <font color='#FF0000'> &lt;ad&gt; </font> tag was found in the VAST file. Invalid VAST file."});
							return;
						}else{
							...
							for(var i=0; i< respObj["Ad"].length; i++){
								...
								if(!respObj["Ad"][i]["InLine"]){
									data.dispatchEvent(TLSVPData.LOAD_ERROR, {text:"No <font color='#FF0000'> &lt;InLine&gt; </font>tag was found in the VAST xml file."});
									return;
								}
								tempObj["InLine"] = {};
										...
										if(respObj["Ad"][i]["InLine"]["Creatives"]["Creative"][j]["Linear"]){
											...
											if(tempObj["InLine"]["Linear"]["skipoffset"]){
												...
												if(tempObj["InLine"]["Linear"]["Duration"] && tempObj["InLine"]["Linear"]["skipoffset"].indexOf("%") != -1){
													var tempSkipOffset =  Math.round(TLSVPUtils.getSecondsFromString(tempObj["InLine"]["Linear"]["Duration"]) * (tempObj["InLine"]["Linear"]["skipoffset"].substr(0, tempObj["InLine"]["Linear"]["skipoffset"].length -1)/100));
													tempObj["InLine"]["Linear"]["skipoffset"] = TLSVPUtils.formatTime(tempSkipOffset, true);
												}
												
											}
											
											if(tempObj["InLine"]["Linear"]["skipoffset"]){
												tempObj["InLine"]["Linear"]["skipoffset"] = TLSVPUtils.getSecondsFromString(tempObj["InLine"]["Linear"]["skipoffset"]);
												if(tempObj["InLine"]["Linear"]["Duration"] && TLSVPUtils.getSecondsFromString(tempObj["InLine"]["Linear"]["Duration"]) <= tempObj["InLine"]["Linear"]["skipoffset"]){
													tempObj["InLine"]["Linear"]["skipoffset"] = undefined;
												}
											}
						...
						TLSVPUtils.storArrayBasedOnObjectValue(linearVast_ar, "sequence");
						TLSVPUtils.storArrayBasedOnObjectValue(nonLinearVast_ar, "sequence");
						...
						for(var i=0; i<nonLinearVast_ar.length; i++){
							...
							if(nonLinearVast_ar[i].startTime) adsObj.timeStart = TLSVPUtils.getSecondsFromString(nonLinearVast_ar[i].startTime);
							...
							if(_s.vastData_ar.length && _s.vastData_ar[_s.countVastDataLoaded]['timeOffset']){
								...
								if(vmapTimeStart){
									if(vmapTimeStart.toLowerCase() == 'start'){
										...
										adsObj.timeStart = TLSVPUtils.getSecondsFromString(vmapTimeStart.toLowerCase());
									}
								}
							}
						...
						for(var i=0; i<linearVast_ar.length; i++){
							...
							adsObj.timeStart = TLSVPUtils.getSecondsFromString(data.vastLinearStartTime);
							if(linearVast_ar[i].startTime) adsObj.timeStart = TLSVPUtils.getSecondsFromString(linearVast_ar[i].startTime);
							if(!adsObj.timeStart) adsObj.timeStart = 0;
						...
						if(_s.countVastDataLoaded == _s.totalDataVastToLoad){
							data.dispatchEvent(TLSVPData.VAST_LOADED, {ads:data.adsSource_ar, popupAds:data.popupAds_ar});
						}else{
							_s.load(_s.vastData_ar[_s.countVastDataLoaded]['source']);
						}
					}else{
						data.dispatchEvent(TLSVPData.LOAD_ERROR, {text:"vast XML file can't be loaded " +  _s.vastXHR.statusText});
					}
				}
			};
		...
		_s.fixVmapTimes = function(duration, curAddData, curPopupAdsData, id){
			...
			if(curPopupAdsData){
				for(var i=0; i<curPopupAdsData.length; i++){
					...
					ad['timeEnd'] = ad['timeStart'] + TLSVPUtils.getSecondsFromString(ad['duration']);
					if(i > 0){
	...
	/* set prototype */
	TLSVPVast.setPrototype = function(){
		TLSVPVast.prototype = null;
		TLSVPVast.prototype = new TLSVPEventDispatcher("div");
	};
	TLSVPVast.prototype = null;
	window.TLSVPVast = TLSVPVast;
}(window));