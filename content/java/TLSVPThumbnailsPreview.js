 ...
 */
(function (window){
var TLSVPThumbnailsPreview = function(
		controller
		){
		...
		var _s = this;
		var prototype = TLSVPThumbnailsPreview.prototype;
		...
		_s.init = function(){
			...
			_s.pointer_do = new TLSVPDisplayObject("div");
		...
		_s.setupCanvas = function(){
			if(_s.canvas) return;
			_s.canvas_do = new TLSVPDisplayObject("canvas");
		...
		_s.load = function(path, videoType, videoSource, video){
			if(path == 'auto'){
				...
				if(_s.videoType == TLSVPlayer.VIDEO){
					_s.setupVideo();
				}else if(_s.videoType == TLSVPlayer.HLS_JS){
					_s.setupVideo();
					if(window['Hls']){
						_s.hlsJS = new Hls();
		...
		_s.onLoad = function(e){
			...
			if(_s.xhr.readyState == 4){
				if(_s.xhr.status == 404){
					_s.dispatchEvent(TLSVPData.LOAD_ERROR, {text:"Thumbnails preview .vtt file not found: <font color='#FF0000'>" + _s.sourceURL_str + "</font>"});
				}else if(_s.xhr.status == 408){
					_s.dispatchEvent(TLSVPData.LOAD_ERROR, {text:"Loadiong thumbnails preview .vtt file file file request load timeout!"});
			...
			_s.dispatchEvent(TLSVPThumbnailsPreview.LOAD_COMPLETE);
		};
		...
		_s.onError = function(e){
			...
			_s.dispatchEvent(TLSVPThumbnailsPreview.LOAD_ERROR, {text:"Error loading thumbnails preview .vtt file : <font color='#FF0000'>" + _s.sourceURL_str + "</font>."});
		};
		...
		_s.parseVtt = function(file_str){
		    ...
		    for(var s in srt_) {
		        ...
		        if(st.length >=2) {
		            ...
		            _s.vtt_ar[cont].startDuration = TLSVPUtils.formatTimeWithMiliseconds(_s.vtt_ar[cont].start);
		            _s.vtt_ar[cont].endDuration = TLSVPUtils.formatTimeWithMiliseconds(_s.vtt_ar[cont].end);
		        }
		        cont++;
		    }
	...
	/* set prototype */
	TLSVPThumbnailsPreview.setPrototype = function(){
		TLSVPThumbnailsPreview.prototype = null;
		TLSVPThumbnailsPreview.prototype = new TLSVPDisplayObject("div");
	};
	
	TLSVPThumbnailsPreview.LOAD_ERROR = 'loadError';
	TLSVPThumbnailsPreview.LOAD_COMPLETE = 'loadComplete';
	TLSVPThumbnailsPreview.START_TO_SCRUB = "startToScrub";
	TLSVPThumbnailsPreview.SCRUB = "scrub";
	TLSVPThumbnailsPreview.STOP_TO_SCRUB = "stopToScrub";
	TLSVPThumbnailsPreview.prototype = null;
	window.TLSVPThumbnailsPreview = TLSVPThumbnailsPreview;
}(window));