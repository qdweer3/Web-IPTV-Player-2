...
(function (window){
	...
	var TLSVPPassword = function(_d, prt, lg){

		'use strict';
		
		var _s = this;
		var prototype = TLSVPPassword.prototype;
		...
		_s.isMobile_bl = TLSVPUtils.isMobile;
		...
		_s.init = function(){
			...
			_s.bk_do = new TLSVPDisplayObject("div");
			_s.bk_do.getStyle().width = "100%";
			_s.bk_do.getStyle().height = "100%";
			_s.bk_do.setAlpha(.9);
			_s.bk_do.getStyle().background = "url('" + _s.embedWindowBackground_str + "')";
		
			_s.passMainHolder_do =  new TLSVPDisplayObject("div");
			
			_s.passMainHldBk = new TLSVPDisplayObject("div");
			...
			_s.passMainLabel_do = new TLSVPDisplayObject("div");
			...
			_s.passLabel_do = new TLSVPDisplayObject("div");
			_s.passMainLabel_do.screen.className = 'UVP-main-label';
			...
			_s.passInput_do = new TLSVPDisplayObject("input");
			...
			if(!lg){
				TLSVPSimpleSizeButton.setPrototype();
				_s.passBtn = new TLSVPSimpleSizeButton(
				...
				_s.passBtn.addListener(TLSVPSimpleSizeButton.CLICK, _s.passClickHandler);
			
				//setup close button
				TLSVPSimpleButton.setPrototype();
				if(_s.useVectorIcons_bl){
					_s.clsBtn = new TLSVPSimpleButton(
							0, 0, 0, true, 0, 0, 0,
							"<div class='table-telasevp-button'><span class='table-cell-telasevp-button fwdicon-close'></span></div>",
							undefined,
							"UVPCloseButtonNormalState",
							"UVPCloseButtonSelectedState"
					);
					_s.clsBtn.addListener(TLSVPSimpleButton.MOUSE_UP, _s.closeButtonOnMouseUpHandler);
				...
				_s.clsBtn.screen.className = 'telasevp-close-button';
				_s.clsBtn.addListener(TLSVPSimpleButton.MOUSE_UP, _s.closeButtonOnMouseUpHandler);
				...
				var clsn = 'telasevp-loggedin-message-white';
				if(_d.isDark){
					clsn = 'telasevp-loggedin-message-dark';
				}
		...
		function selectText(){
			if(window.top != window && TLSVPUtils.isIE) return;
			var range, selection;
		...
		_s.passClickHandler = function(){
			...
			if(playlistPass){
				if(playlistPass != TLSVPUtils.MD5(_s.passInput_do.screen.value)){
					allow = false;
				}else{
					prt.plPassPassed = true;
				}
			}else if(vidPass){
				if(vidPass != TLSVPUtils.MD5(_s.passInput_do.screen.value)){
					allow = false;
				}
			}else{
				if(_s.privateVideoPassword_str != TLSVPUtils.MD5(_s.passInput_do.screen.value)){
					allow = false;
				}
			}
			if(!allow){
			...
			_s.dispatchEvent(TLSVPPassword.CORRECT);
		};
		...
		_s.show = function(id){
			...
			if(!TLSVPUtils.isMobile || (TLSVPUtils.isMobile && TLSVPUtils.hasPointerEvent)) prt.main_do.setSelectable(true);
		...
		_s.hide = function(){
			...
			if(!TLSVPUtils.isMobile || (TLSVPUtils.isMobile && TLSVPUtils.hasPointerEvent)) prt.main_do.setSelectable(false);
		...
		_s.hideCompleteHandler = function(){
			...
			_s.dispatchEvent(TLSVPPassword.HIDE_COMPLETE);
		};
	...
	/* set prototype */
	TLSVPPassword.setPrototype = function(){
		TLSVPPassword.prototype = new TLSVPDisplayObject("div");
	};
	
	TLSVPPassword.ERROR = "error";
	TLSVPPassword.CORRECT = "correct";
	TLSVPPassword.HIDE_COMPLETE = "hideComplete";
	
	TLSVPPassword.prototype = null;
	window.TLSVPPassword = TLSVPPassword;
}(window));