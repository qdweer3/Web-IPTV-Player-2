...
var TLSVPCC = function(
		...
		var _s = this;
		var prototype = TLSVPCC.prototype;
		...
		_s.initializeController =  function(){
			controller.addListener(TLSVPController.UNCAST, _s.stopCastingHandler);
		}
		...
		_s.initAPI = function(){
			...
			TLSVPlayer.keyboardCurInstance = main;
		...
		_s.setupCastingScreen =  function(){
			_s.cs_do = new TLSVPDisplayObject("div");
		...
		_s.checkButtonState = function(){
			...
			if(main.videoType_str != TLSVPlayer.VIDEO && main.videoType_str != TLSVPlayer.MP3 && main.videoType_str != TLSVPlayer.HLS_JS){
		...
		_s.isValidFormat = function(){
			if(main.videoType_str == TLSVPlayer.VIDEO || main.videoType_str == TLSVPlayer.MP3 || main.videoType_str == TLSVPlayer.HLS_JS) return true;
			return false;
		}
					...
					if(_s.remotePlayer.isConnected){
						...
						if(_s.playerState == PLAYER_STATE.PLAYING && !_s.isMbl
						  && _s.videoSource == main.finalVideoPath_str){
							var curTime;
							curTime = TLSVPUtils.formatTime(_s.currentTime);
		...
		_s.addPlayerEvents =  function(){
			...
			_s.remotePlayerController.addEventListener(
				...
				function (event){
					var time = TLSVPUtils.formatTime(_s.getCurrentTime()) + "/" + TLSVPUtils.formatTime(_s.getDuration());
		...
		_s.loadMedia = function(autoplay){
			...
			_s.videoSource = TLSVPUtils.getValidSource(main.finalVideoPath_str);
			var posterSource = TLSVPUtils.getValidSource(main.posterPath_str);
			
			var mediaInfo = new chrome.cast.media.MediaInfo(_s.videoSource);
			mediaInfo.metadata = new chrome.cast.media.GenericMediaMetadata();
			var ct = 'video/mp4';
			if(main.videoType_str == TLSVPlayer.MP3){
				ct = 'audio/mp3';
			}
			...
			if(subData){
				...
				for(var i=0; i<subData.length - 1; i++){
					...
					track.trackContentId = TLSVPUtils.getValidSource(subData[i]['source']);
				...
				track.trackContentId = TLSVPUtils.getValidSource('content/subtitles/empty.vtt');
				track.subtype = chrome.cast.media.TextTrackType.SUBTITLES;
				track.name = '';
				track.customData = null;
				track.trackContentType = 'text/vtt';
	...
	/* set prototype */
	TLSVPCC.setPrototype = function(){
		TLSVPCC.prototype = null;
		TLSVPCC.prototype = new TLSVPEventDispatcher("div");
	};

	TLSVPCC.prototype = null;
	window.TLSVPCC = TLSVPCC;
}(window));