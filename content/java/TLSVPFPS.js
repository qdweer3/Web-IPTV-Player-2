 ...
 */
(function (window){
	var TLSVPFPS = function(prt){

		'use strict';

		var _s = this;
		var prototype = TLSVPFPS.prototype;
		...
		_s.setupText = function(){
			_s.txt = new TLSVPDisplayObject("div");
			_s.txt.getStyle().display = 'inline-block';
			_s.txt.hasTransform3d_bl = false;
			_s.txt.hasTransform2d_bl = false;
			var txt = '<div class="telasevp-fingerprintstamp-holder">'
			var obj = window['telasevpFingerPrintStamp'];
			for (var prop in obj){
				txt += prop + ' ' + obj[prop];
			}
			txt += '</div>';
	...
	/* set prototype */
	TLSVPFPS.setPrototype = function(){
		TLSVPFPS.prototype = null;
		TLSVPFPS.prototype = new TLSVPDisplayObject("div");
	};


	TLSVPFPS.prototype = null;
	window.TLSVPFPS = TLSVPFPS;
}(window));
